...
LOCATION = "us-east4" # Recommended region
app = Flask(__name__)
...
def chat():
    ...
    # We now store a JSON-serializable list of dictionaries
    chat_history_dicts = session.get('chat_history', [])
    
    # Reconstruct the ChatSession from the stored history
    # This is the key change to make the session work correctly
    ...
    try:
        ...
        session['chat_history'] = [
            {'role': turn.role, 'parts': [{'text': part.text} for part in turn.parts]}
